...
int main(int argc, char *argv[]){
    ...
    if(user_text < 0){
        error("otp_dec ERROR cant open argv1");
    }
    ...
    if(key_text < 0){
        error("otp_dec ERROR cant open argv2");
    }
    ...
    {
        error("otp_dec ERROR key is too short");
    }
    //---------------------END OF INPUT CHECK-------------------///
    ...
    if (r < 0) {
        error("otp_dec: ERROR reading handshake");
    }
    if(strcmp(handshake_back, "success") != 0)
    {
        fprintf(stderr, "1otp_enc: ERROR could not contact client on port %d\n", portNumber);
        exit(2);
    }
    //---------------------END- HANDSHAKE-------------------///
    ...
    if(strcmp(handshake_back, "success") != 0)
    {
        fprintf(stderr, "2otp_enc: ERROR could not contact client on port %d\n", portNumber);
        exit(2);
    }
    
    send_size = send(listenSocketFD, &user_text_size, sizeof(user_text_size), 0); //send message length
    if(send_size < 0){ error("ERROR Sending key size");}
    //---------------------END OF SEND MESSAGE/KEY SIZE-------------------///

    //quick handshake
    memset(handshake_back, '\0', sizeof(handshake_back));//clear the buffer
    r = recv(listenSocketFD, handshake_back, sizeof(handshake_back) - 1,0);//check if handshake successful
    if (r < 0) { error("otp_dec: ERROR reading handshake");}
    if(strcmp(handshake_back, "success") != 0)
    {
        fprintf(stderr, "otp_dec: ERROR could not contact client on port %d\n", portNumber);
        exit(2);
    }
    ...
    if (send_size < 0) { error("otp_dec: Error sending message to server\n");}
    ...
    if(strcmp(handshake_back, "success") != 0)
    {
        fprintf(stderr, "4otp_enc: ERROR could not contact client on port %d\n", portNumber);
        exit(2);
    }
    
    // Send key to server
    send_size = send(listenSocketFD, key_buffer, strlen(key_buffer), 0); // Write to the server
    if (send_size < 0) { error("otp_dec: Error sending key to server\n");}
    //---------------------END  MESSAGE/KEY-------------------///

    
    // Get return encyrpted from server
    memset(ciphertext, '\0', sizeof(ciphertext)); // Clear out the buffer again
    charsRead = recv(listenSocketFD, ciphertext, user_text_size - 1, 0); // Read data from the socket, leaving \0 at end
